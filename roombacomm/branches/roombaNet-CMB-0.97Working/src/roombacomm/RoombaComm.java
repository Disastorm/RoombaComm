 ...
 *
 * @author Tod E. Kurt
 * SVN id value is $Id: RoombaComm.java 54 2009-08-18 04:14:35Z black.123 $
 */
public abstract class RoombaComm
{
    /** version of the library */
    static public final String VERSION = "0.96.2";
...
//        send( CLEAN );
     ...
     */
    // this method should be private and setLEDs(roombacomm) should be public
    private void setLEDs( boolean status_green, boolean status_red, 
    ...
    //500 series
    // this method should be private and setLEDs(roombacomm) should be public
    private void setLEDsOI( boolean checkRobot, boolean spot,boolean dock,boolean dirt, 
        int power_color, int power_intensity ) {
...
spinright = -127 83
	...
	public void setLEDs(RoombaComm roombacomm) {
		...
		roombacomm.setLEDs(this.isGreenOn(), this.isRedOn(), this.isToggleSpot(), this.isToggleClean(), this.isToggleMax(), this.isToggleDirt(), 
				this.getPower_color(), this.getPower_int());
		}
		if (this.protocol.equalsIgnoreCase("OI")){
			/*
			 *  I think the API docs might have a but in them.
			 *  the signature for the OI and the SCI Opcode: 139 are very similar, but also very different
			 *  I think the setLEDsOI maps to the OI docs, but it appears to not be working.
			 *  So I commmented it out and mapped the OI LED's into the mapped bits for the SCI API call. (Just to see if it works out any better.)
			 *  If it does work then this long comment block could (and should) be removed
			 *  
			 ...
			 *  
			 */
			roombacomm.setLEDs(this.isGreenOn(), this.isRedOn(), this.isToggleCheckRobot(), this.isToggleDock(), this.isToggleSpot(), this.isToggleDirt(), this.getPower_color(), this.getPower_int());
			updateDisplay("Checkrobot("+this.isToggleCheckRobot() +"),Spot("+ this.isToggleSpot() +"),Dock("+ this.isToggleDock() +"),Dirt("+ this.isToggleDirt() +"),Pcolor("+ this.getPower_color() +"),Pint("+ this.getPower_int()+")",true);
		}
	}

	public void setChgGreenLED(RoombaComm roombacomm, boolean green) {
		this.setGreenOn(green);
	...
	public void setToggleSpot(boolean toggleSpot) {
		this.toggleSpot = toggleSpot;
	}

	public void setToggleClean(boolean toggleClean) {
		this.toggleClean = toggleClean;
	}

	public void setToggleMax(boolean toggleMax) {
		this.toggleMax = toggleMax;
	}

	public void setToggleDirt(boolean toggleDirt) {
		this.toggleDirt = toggleDirt;
	}

	public void setToggleCheckRobot(boolean toggleCheckRobot) {
		this.toggleCheckRobot = toggleCheckRobot;
	}

	public void setToggleDock(boolean toggleDock) {
		this.toggleDock = toggleDock;
	}

	public void setPower_color(int power_color) {
	...
	*/
	...
	public int getPower_color() {
		return power_color;
	}

	public void setPower_int(int power_int) {
		this.power_int = power_int;
	}

	public int getPower_int() {
		return power_int;
	}
